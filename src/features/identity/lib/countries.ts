/**
 * ISO 3166-1 alpha-2, English name, and E.164 calling code. Names came from
 * `Intl.DisplayNames` at authoring time rather than being hand-typed; the flag
 * is derived from the code in `flagOf`, so no emoji is stored here.
 */
export type Country = {
...
export const COUNTRIES: readonly Country[] = [
  ...
  { iso2: 'AG', name: 'Antigua & Barbuda', dial: '1' },
  ...
  { iso2: 'ST', name: 'São Tomé & Príncipe', dial: '239' },
  ...
  { iso2: 'BL', name: 'St. Barthélemy', dial: '590' },
  { iso2: 'SH', name: 'St. Helena', dial: '290' },
  { iso2: 'KN', name: 'St. Kitts & Nevis', dial: '1' },
  { iso2: 'LC', name: 'St. Lucia', dial: '1' },
  { iso2: 'MF', name: 'St. Martin', dial: '590' },
  { iso2: 'PM', name: 'St. Pierre & Miquelon', dial: '508' },
  { iso2: 'VC', name: 'St. Vincent & Grenadines', dial: '1' },
  { iso2: 'SD', name: 'Sudan', dial: '249' },
  { iso2: 'SR', name: 'Suriname', dial: '597' },
  { iso2: 'SJ', name: 'Svalbard & Jan Mayen', dial: '47' },
  ...
  { iso2: 'TC', name: 'Turks & Caicos Islands', dial: '1' },
  ...
  { iso2: 'WF', name: 'Wallis & Futuna', dial: '681' },
...
/** Two regional-indicator symbols render as that country's flag. */
...
/** Matches on name or dial code, so "+92" and "pak" both find Pakistan. */
export function searchCountries(query: string): readonly Country[] {
  ...
  return COUNTRIES.filter(
    (country) => country.name.toLowerCase().includes(trimmed) || country.dial.startsWith(trimmed),
  );
}
